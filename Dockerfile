...
RUN apk upgrade --no-cache -a && \
    ...
    #--enable-curl --enable-session-ticket --enable-earlydata --enable-psk --enable-harden --enable-altcertchains --enable-opensslextra
    /src/wolfssl/configure CFLAGS="-DWOLFSSL_NO_ASN_STRICT" --prefix=/usr/local --enable-quic --enable-ech --disable-shared --enable-static && \
    make -j "$(nproc)" && \
